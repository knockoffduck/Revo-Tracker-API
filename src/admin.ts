...
const mapPbUser = (record: any) => ({
	...
	gymPreferences: record.gym_preferences ?? null,
});
...
app.get("/admin/users", async (c) => {
	try {
		...
		const page = Math.floor(Math.max(Number(c.req.query("offset") ?? 0), 0) / limit) + 1;
		const sort = c.req.query("sort") ?? "email";
		const order = c.req.query("order") === "asc" ? "+" : "-";

		// PocketBase's built-in users collection cannot be sorted by the system
		// `created`/`updated` fields, so only name/email are allowed.
		const allowedSortFields = ["name", "email"];
		const sortField = allowedSortFields.includes(sort) ? sort : "email";

		let filter = "";
		if (q) {
			filter = `(name~'${q.replace(/'/g, "\\'")}' || email~'${q.replace(/'/g, "\\'")}')`;
		}
		...
		return c.json({
			success: true,
			data: {
				users: result.items.map(mapPbUser),
				pagination: {
					limit,
					offset: (page - 1) * limit,
...
app.patch("/admin/users/:id", async (c) => {
	try {
		...
		if (updates.is_admin === false) {
			const current = await pb.collection("users").getOne(id);
			if (current.is_admin) {
		...
		return c.json({ success: true, data: mapPbUser(updated) });
	} catch (err: any) {
		if (err?.status === 404) {
...
app.delete("/admin/users/:id", async (c) => {
	try {
		...
		const target = await pb.collection("users").getOne(id);
		if (target.is_admin) {
			const admins = await pb.collection("users").getList(1, 1, {
				filter: "is_admin=true",
			});
